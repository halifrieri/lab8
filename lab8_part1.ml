...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      |Empty, _
      |_, Empty -> Empty
      |Interval (a,b), Interval (c,d) -> create (if Endpoint.compare a c = 1 then a else c)
                                                (if Endpoint.compare b d = -1 then b else d)
    end ;;
...
module IntInterval = MakeInterval (Ordered_type) ;;
